...
async function showFilteredTowns(input) {
  ...
  const newArr = towns.filter((item) => isMatching(item.name, input));
  const fragment = document.createDocumentFragment();

  newArr.forEach((item) => {
    const townDiv = document.createElement('div');
    townDiv.textContent = item.name;
