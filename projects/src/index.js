...
function collectDOMStat(root, obj = {}) {
  for (let i = root.childNodes.length - 1; i >= 0; i--) {
    ...
    if (item.nodeType === 3) {
      ...
      if (obj.tags[item.tagName] === undefined) {
        obj.tags[item.tagName] = 0;
      }
      ...
      for (let j = 0; j < item.classList.length; j++) {
        const className = item.classList[j];
        if (obj.classes[className] === undefined) {
          obj.classes[className] = 0;
        }
